...
def vala_file(self, node):
	...
	if not valatask:
		...
		if valatask.is_lib:
			valatask.outputs.append(self.path.find_or_declare('%s.h' % self.name))
...
@conf
def check_vala(self, min_version=(0,18,0), branches=('0.20', '0.18')):
    """
...
"""
    ...
    for branch in branches:
        try:
            valac_name = 'valac-'+branch
            valac = self.find_program(valac_name, var='VALAC')
        except:
            continue
        break

    # No acceptable branches found, try valac by itself, fail if not found
    if not valac:
        valac_name = 'valac'
        valac = self.find_program(valac_name, var='VALAC')
