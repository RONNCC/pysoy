...
class soy.bodies.Billboard : soy.bodies.Body
    ...
    construct (position : soy.atoms.Position?, size : soy.atoms.Size?, material : soy.materials.Material?)
    ...
    def override render ( )
        ...
        while self._material.render(i, null)
            glDrawElements(GL_TRIANGLES, (GLsizei) (_xsubdivisions*_ysubdivisions*6), GL_UNSIGNED_BYTE, (GLvoid*) 0)
            i += 1
    ...
    def _update_billboard()
        ...
        vertices : array of GLfloat = new array of GLfloat[(1+_xsubdivisions)*(1+_ysubdivisions)*11]

        for var i=0 to _ysubdivisions
            for var j=0 to _xsubdivisions
                vertices[(i*(_xsubdivisions+1)+j)*11] = (_width * j / _xsubdivisions) - (_width/2)
                vertices[(i*(_xsubdivisions+1)+j)*11+1] = (_height * i / _ysubdivisions) - (_height/2)
        ...
        // bind elements
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _ebo)
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, (GLsizei) (6 * _xsubdivisions * _ysubdivisions * sizeof(GLubyte)),
                     elements, GL_STATIC_DRAW)

        // bind vertices
        glBindBuffer(GL_ARRAY_BUFFER, _vbo)
        glBufferData(GL_ARRAY_BUFFER, (GLsizei) (11 * (1+_xsubdivisions) * (1+_ysubdivisions) * sizeof(GLfloat)),
