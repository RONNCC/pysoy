    ...
    */
    ...
    def new get (index : int) : soy.atoms.Face?
        ...
        if self._face_objs.has_key(index)
            ...
            v : soy.atoms.Vertex
            // each vertex occupies 12 indices and there are three vertices per face
    ...
    def clone() : Mesh
        // clone spins off a new mesh identical to the current one (used when creating mesh morphs)
        // because Mesh does not worry about the VBO or materials lists unless/until render() is called, this will be relatively cheap
        //mesh : Mesh = new Mesh(self.scene)
        mesh : Mesh = new Mesh(position)
        //TODO we need on_set events that will update length for the clone whenever it is updated in self
        // also, when new faces are added (but NOT when existing faces are updated) the face should be added to the clone also
    ...
    def morph(variant : soy.bodies.Mesh, delta : float) : soy.atoms.Morph
        // morph spins off a morph atom, which can be added to the list of morphs to be applied to a Target
