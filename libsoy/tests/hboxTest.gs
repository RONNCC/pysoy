/*
 *  libsoy tests - hboxTest (hboxTest.gs)
...
init
    ...
    var camera2 = new soy.bodies.Camera.with_position(scene2, {0,0,10}, {0,0,0})
    ...
    // Create a horizontal box inside the window 
    var hbox = new soy.widgets.HBox(window)
    hbox.size = bigger


    // Connect each camera to the HBox via a projector.
    var projector = new soy.widgets.Projector(hbox, camera)
    var projector2 = new soy.widgets.Projector(hbox, camera2)
    //projector.size = bigger
    //projector2.size = bigger
    
    print "HBox size: %fx%f", hbox.size.width, hbox.size.height
    ...
    hbox.size = new soy.atoms.Size(window.size.width, window.size.height, 0.0)
