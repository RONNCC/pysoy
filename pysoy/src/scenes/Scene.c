...
static Py_ssize_t
sq_length (SELF self) {
    return gee_abstract_map_get_size((GeeAbstractMap*) self->g->bodies);
}
...
sq_contains (SELF self, PyObject* key) {
    ...
    return gee_abstract_map_has_key((GeeAbstractMap*)
                                    self->g->bodies, PyBytes_AsString(name));
}
...
mp_subscript (SELF self, PyObject* key) {
    ...
    if (!gee_abstract_map_has_key((GeeAbstractMap*) self->g->bodies,
