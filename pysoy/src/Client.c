...
PySoy_Client_init (PySoy_Client_Object* self, PyObject* args, PyObject* kwds) {
    ...
    if (!self->window)
        return -1;
    soyWindow = soy_widgets_window_new(NULL);
...
PySoy_Client_pointer_getter (PySoy_Client_Object* self, void* closure) {
	...
	ret = (PySoy_controllers_Pointer_Object*)
		PyType_GenericNew(g_type_get_qdata(G_OBJECT_TYPE(self->pointer), GPyObject),
						  NULL, NULL);
...
PySoy_Client_keyboard_getter (PySoy_Client_Object* self, void* closure) {
	...
	ret = (PySoy_controllers_Keyboard_Object*)
		PyType_GenericNew(g_type_get_qdata(G_OBJECT_TYPE(self->keyboard), GPyObject),
						  NULL, NULL);
...
PySoy_Client_thread_init (void) {
    ...
    PySoy_Client_thread = g_thread_create(PySoy_Client_thread_loop, NULL,
                                          FALSE, &error);
...
static int PySoy_Client_set_WM_hints(PySoy_Client_Object* self) {
	...
	// Setting the max and min dimension hints to the same values disables resizing
	if(!soy_widgets_window_get_resizable((gpointer) soyWindow)) {
		window_size_hints->flags = PSize | PMinSize | PMaxSize;
		window_size_hints->max_width = soy_atoms_size_get_width(self->window_size);
		window_size_hints->max_height = soy_atoms_size_get_height(self->window_size);
	}
