...
#include "pysoy2.h"
#define SELF PySoy_atoms_Position_Object*


static char
tp_doc[] = "soy.atoms.Position Type\n"
"\n"
"    This type stores the position of an object as three floats (x, y, z).\n"
"\n"
"    Wherever a Position may be used, a sequence of 2 or 3 items may be used\n"
"    instead.  This is primarily a return value which modifies the object\n"
"    that created it when changed, for example:\n"
"\n"
"        >>> p = soy.atoms.Position((10, 10, 10))\n"
"        >>> p\n"
"        <Position (10.000000, 10.000000, 10.000000)>\n"
"        >>> p.x += 10\n"
"        >>> p\n"
"        <Position (20.000000, 10.000000, 10.000000)>\n"
"\n"
"    If window.position returned a plain list changing one of its items\n"
"    would not move the window by itself, you would instead have to:\n"
"\n"
"        window.position = (window.position[0] += 10, window.position[1], 0)\n"
"\n"
...
tp_new (PyTypeObject* type, PyObject* args, PyObject* kwds) {
    ...
    if (!_PyArg_NoKeywords("soy.atoms.Position", kwds));
    ...
    // new gobject
    self->g = soy_atoms_position_new(x, y, z);
...
tp_repr (SELF self) {
    ...
    str = g_strdup_printf("<Position (%f, %f, %f)>",
...
tp_richcompare (PyObject *left, PyObject *right, int op) {
    ...
    if (op == Py_EQ) {
        if (soy_atoms_position_cmp_eq(c1, c2))
            return Py_True;
    }
    else if (op == Py_NE) {
        if (soy_atoms_position_cmp_ne(c1, c2))
            return Py_True;
    }
    else {
        PyErr_SetString(PyExc_NotImplementedError,
                        "only == and != are supported for soy.atoms.Position");
        return NULL;
    }
    return Py_False;
}
...
nb_inplace_add (PyObject* this, PyObject* o2) {
    SELF self = (SELF) this;
    double o2x, o2y, o2z = 0;
	PySoy_atoms_Position_Object *so2;
    ...
    soy_atoms_position_set_x(self->g, soy_atoms_position_get_x(self->g) + o2x);
    soy_atoms_position_set_y(self->g, soy_atoms_position_get_y(self->g) + o2y);
    soy_atoms_position_set_z(self->g, soy_atoms_position_get_z(self->g) + o2z);
...
sq_item(SELF self, Py_ssize_t index) {
    if (index == 0)
        return PyFloat_FromDouble(soy_atoms_position_get_x(self->g));
    if (index == 1)
        return PyFloat_FromDouble(soy_atoms_position_get_y(self->g));
    if (index == 2)
        return PyFloat_FromDouble(soy_atoms_position_get_z(self->g));
...
sq_ass_item(SELF self, Py_ssize_t index, PyObject* value) {
    ...
    if (index == 0)
        soy_atoms_position_set_x(self->g, PyFloat_AsDouble(value));
    else if (index == 1)
        soy_atoms_position_set_y(self->g, PyFloat_AsDouble(value));
    else if (index == 2)
        soy_atoms_position_set_z(self->g, PyFloat_AsDouble(value));
    else {
        PyErr_SetString(PyExc_IndexError, "Position index out of range");
        return -1;
    }
...
static char
x_doc[] = "Position on X dimension";
PYSOY_PROP_FLOAT(atoms, position, x)


static char
y_doc[] = "Position on Y dimension";
PYSOY_PROP_FLOAT(atoms, position, y)


static char
z_doc[] = "Position on Z dimension";
PYSOY_PROP_FLOAT(atoms, position, z)
...
PyTypeObject PySoy_atoms_Position_Type = {
    ...
    "soy.atoms.Position",                                  // tp_name
    sizeof(PySoy_atoms_Position_Object),                   // tp_basicsize
    0,                                                     // tp_itemsize
    0,                                                     // tp_dealloc
    0,                                                     // tp_print
